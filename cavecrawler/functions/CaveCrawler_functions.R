...
QTL <- function(chr_table, position_table, QTL_table, 
  ...
  if (GR.bool) {
    ...
    }else if(nrow(plot_annot) == 1){
      QTL.wrnings <- append(QTL.wrnings, 
                            paste(c("No markers or genes found within range of ",
                                    plot_annot$`Element Name`[1], 
                                    ". No plot outputted.", "; "), collapse = ""))
    }else{
      QTL.wrnings <- append(QTL.wrnings, 
                            "No markers or genes found within inputted range; ")
    }
  ...
  if(TM.bool){
    ...
    for(i in 1:length(TM_markers)){
      ...
      if(TM_markers[i] %in% QTL_table$Marker){
        ...
        QTL.wrnings <- append(QTL.wrnings, 
                              paste(c("Marker data not present for trait ", 
                                      TM_markers[i], "; "), collapse = ""))
      }
    }
